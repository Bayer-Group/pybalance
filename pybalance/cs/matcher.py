...
def _rescale_for_discretization(
    ...
    min_value = min(np.abs(target[target != 0]).min(), np.abs(pool[pool != 0]).min())
    while 1:
        min_value_scaled = min_value * scalefac
        truncation_error = (
            np.abs(min_value_scaled - min_value_scaled.astype(int)) / min_value_scaled
        )
    ...
    scalefac *= 2
    logger.info(
        f"Scaling features by factor {scalefac:.2f} in order to use integer solver with <= {100 * tolerance}% loss."
    )
    logger.info(
        f"With this scaling, the smallest value {min_value} is represented by {(min_value * scalefac).astype(int)}."
    )
...
class ConstraintSatisfactionMatcher(object):
    ...
    :param parallel_solve: Whether to use multiple threads in the backend
        solver.
    ...
    def __init__(
        ...
        self.target, self.pool = _rescale_for_discretization(
            target * self.weights, pool * self.weights, tolerance=0.001
        )
    ...
    def match(self, hint: Optional[List[int]] = None) -> MatchingData:
        ...
        if self.parallel_solve:
            ...
            solver.parameters.num_search_workers = min(8, multiprocessing.cpu_count())
        ...
        logger.info(
            "Solving with %d workers ..." % solver.parameters.num_search_workers
        )
        # ========= Solve===========#
